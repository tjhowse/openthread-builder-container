...
This project implements the build environment for the following incomplete, inaccurate and outdated guide:
...
2) Install podman, run go.sh to build and launch the container. The same would probably work for docker with slight tweaks. You may have to adjust the `--device /dev/ttyACM0` bit to match your device.
...
You may have issues with the wifi settings not applying. Try using the serial console to use the `wifi connect` command.
